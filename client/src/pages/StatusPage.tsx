...
import { ShieldAlert, Compass, ArrowLeft, Home, FileText } from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import { normalizeRole, dashboardPathForRole } from '../lib/roles';

interface Props {
  kind: 'unauthorized' | 'notFound' | 'comingSoon';
  /** Page name, used by the comingSoon variant. */
  title?: string;
}
 ...
 */
const StatusPage = ({ kind, title }: Props) => {
  const { user } = useAuth();
  const navigate = useNavigate();
  const isForbidden = kind === 'unauthorized';
  const isComingSoon = kind === 'comingSoon';
    ...
    <div className="flex min-h-screen items-center justify-center bg-bg-canvas p-6">
      <div className="w-full max-w-md rounded-2xl border border-line bg-bg-surface p-8 text-center">
        <div className="mx-auto mb-5 grid h-12 w-12 place-items-center rounded-xl bg-brand-orange/10 text-brand-orange">
          {isComingSoon ? <FileText size={22} /> : isForbidden ? <ShieldAlert size={22} /> : <Compass size={22} />}
        </div>
        ...
        <p className="mt-3 text-sm leading-relaxed text-text-muted">
          {isComingSoon
            ? 'We haven’t published this document yet. If you need it before then, please get in touch and we’ll send it over.'
            : isForbidden
            ? user
