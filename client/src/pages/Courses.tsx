  ...
  return (
    <div className="min-h-screen bg-bg-canvas py-12 text-strong">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {/* Category Tabs */}
        <div className="mb-8 flex justify-center border-b border-line">
          <div className="flex gap-8">
            <button
              onClick={() => setActiveTab('AI')}
              className={clsx(
                "pb-4 text-base font-semibold transition-colors relative",
                activeTab === 'AI' ? "text-brand-orange" : "text-text-muted hover:text-strong"
              )}
            >
            ...
            <button
              ...
              className={clsx(
                "pb-4 text-base font-semibold transition-colors relative",
                activeTab === 'DATABASE' ? "text-brand-orange" : "text-text-muted hover:text-strong"
              )}
            >
        ...
        {loading ? (
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-2">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="h-72 animate-pulse rounded-xl bg-bg-surface border border-line" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-2">
            {courses.map((course) => (
              <div
                key={course.id}
                className="group flex flex-col justify-between rounded-xl border border-line bg-bg-surface p-6 transition-all duration-200 hover:border-brand-orange/40 hover:scale-[1.01]"
              >
                <div>
                  ...
                  <h3 className="mt-4 font-display text-2xl font-bold text-strong group-hover:text-brand-orange transition-colors">
                    {course.title}
                  </h3>
                ...
                <div className="mt-6 flex items-center gap-3 border-t border-line pt-4">
                  <button
                    ...
                    className="flex-1 rounded-lg border border-line-strong px-4 py-2.5 text-center text-sm font-semibold text-strong transition-colors hover:bg-elevate"
                  >
                    Explore Curriculum →
                  </button>
                  <button
                    onClick={() => setSelectedCourseForEnroll(course)}
                    className="flex-1 rounded-lg bg-brand-orange px-4 py-2.5 text-center text-sm font-semibold text-on-brand transition-colors hover:bg-brand-orange/90"
                  >
                    Enroll Now
                  </button>
                </div>
      ...
      {selectedCourseForDrawer && (
        <div className="fixed inset-0 z-50 overflow-hidden bg-overlay backdrop-blur-xs">
            ...
            <div className="w-screen max-w-xl bg-bg-surface border-l border-line p-6 shadow-2xl overflow-y-auto">
              ...
              <div className="flex items-center justify-between border-b border-line pb-4">
                ...
                <button
                  ...
                  className="rounded-lg p-2 text-text-muted hover:bg-elevate hover:text-strong"
                >
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="mt-6 space-y-6">
                <div>
                  <h4 className="text-sm font-semibold text-strong">Description</h4>
                  <p className="mt-1 text-sm text-text-muted">{selectedCourseForDrawer.description}</p>
                </div>

                {selectedCourseForDrawer.careerOutcomes && (
                  <div>
                    <h4 className="text-sm font-semibold text-strong">Career Outcomes</h4>
                ...
                {/* Modules list */}
                <div>
                  <h4 className="mb-3 text-sm font-semibold text-strong">Modules (1 – 6)</h4>
                  <div className="space-y-3">
                    {(selectedCourseForDrawer.modules || []).map((mod, idx) => (
                      <div key={idx} className="rounded-lg border border-line bg-bg-card p-4">
                        <div className="flex items-center justify-between">
                          <h5 className="font-semibold text-strong text-sm">{mod.title}</h5>
                ...
                <div className="pt-4 border-t border-line">
                  <button
                    ...
                    className="w-full rounded-lg bg-brand-orange py-3 text-center text-sm font-semibold text-on-brand shadow-lg hover:bg-brand-orange/90"
                  >
            ...
            {enrollSubmitted ? (
              <div className="py-8 text-center">
                ...
                <p className="mt-2 text-sm text-text-muted">
                  Our academic counselor will reach out to confirm your slot for <strong className="text-strong">{selectedCourseForEnroll.title}</strong>.
                </p>
              </div>
            ) : (
