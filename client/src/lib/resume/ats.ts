...
/* Summary + experience only. These carry the candidate's actual evidence, and
   they are what the ATS score in the editor is measured against — a keyword
   sitting only in a skills list is not the same as one demonstrated in a role. */
export function resumeCoreText(r: ResumeData): string {
  return [r.summary, ...r.experience.flatMap((e) => [e.role, e.company, ...e.bullets])]
    .join(" \n ")
    .toLowerCase();
}

/* A broad, role-agnostic posting used when no specific job description has been
   pasted, so the ATS score always measures real keyword coverage against
   something concrete instead of falling back to a resume-quality checklist. */
...
/** Which part of the resume a score is measured over.
    "all"  — the whole document (what a recruiter's ATS ingests)
    "core" — summary + experience only (the editor's ATS score) */
export type ScoreScope = "all" | "core";

/** Coverage-based score: 50 (parses cleanly) + up to 45 for keyword match. */
export function scoreResume(
  resume: ResumeData,
  keywords: JdKeyword[],
  scope: ScoreScope = "all"
): AtsResult {
  const text = scope === "core" ? resumeCoreText(resume) : resumeToText(resume);
...
export interface SectionCoverage {
  summary: { matched: number; pct: number };
  experience: { matched: number; pct: number };
}

/** Split the keyword coverage across the two sections the ATS score is built
    from, so the UI can show where the score actually comes from. */
export function sectionCoverage(resume: ResumeData, keywords: JdKeyword[]): SectionCoverage {
  ...
  const count = (text: string) => keywords.filter((k) => isCovered(k.label, text)).length;
  const total = keywords.length;
  const pct = (n: number) => (total ? Math.round((n / total) * 100) : 0);

  const s = count(summaryText);
  const e = count(expText);
  return { summary: { matched: s, pct: pct(s) }, experience: { matched: e, pct: pct(e) } };
}
