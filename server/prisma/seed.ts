import { PrismaClient, UserRole, CourseCategory, InternshipType, SubscriptionPlan } from '@prisma/client';
...
async function main() {
  ...
  const newAdminPasswordHash = await bcrypt.hash('pavy@123', 10);
  const newAdmin = await prisma.user.upsert({
    where: { email: 'paulravindra.ai@gmail.com' },
    update: {
      passwordHash: newAdminPasswordHash,
      role: UserRole.ADMIN,
    },
    create: {
      email: 'paulravindra.ai@gmail.com',
      passwordHash: newAdminPasswordHash,
      firstName: 'Paul',
      lastName: 'Ravindra',
      role: UserRole.ADMIN,
  ...
  const internships = [
    {
      ...
      programType: InternshipType.GENERATIVE_AI,
      duration: '12 Weeks',
    ...
    },
    {
      ...
      programType: InternshipType.AGENTIC_AI,
      duration: '12 Weeks',
